...
object CountryBorders {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val countryCallbackFn =
      getCallbackFn(spark, countrySrcFile, delimiter = "\t")()
    ...
    val countryPopulationDF = countryStatsDF.select(
      $"location".as("COUNTRY"),
    ...
    val infection_rate = countryCasesDF
      .join(countryPopulationDF,
            countryCasesDF("COUNTRY") === countryPopulationDF("COUNTRY"))
      .select(
        countryCasesDF("COUNTRY"),
        $"TOTAL CASES",
        $"POPULATION",
  ...
  bcountries have land borders
    ...
    val rankings = dictionaries.developmentRankings.toSeq
      ...
      .select($"ranking", explode($"country").as("country_name"))
