...
object CovidLiveUpdates {
    ...
    def main(args: Array[String]): Unit = {
        ...
        // AWS Access and secret key as arguments 
        // if (args.length <= 2) {
        ...
        // Sets the spark context with all the information to access aws
        ...
        // ** Suppose to run cron job to run q3_data_update.sh or reion_live_update.sh
        while (true) {
            ...
            // Process all the data from the live update
            dataProcessing(spark)
            
            // Force a sleep for 10 minutes
    ...
    /** Processes data and performs union to create single regions DataFrame.
      ...
      */
    def dataProcessing(spark: SparkSession):Unit = {
        // Creates a DataFrame from JSON for each region - From s3
        ...
        // Creates a DataFrame from JSON for each region - locally
        val africaTemp = spark.read.json("datalake/CovidLiveUpdate/africa.json")
        val asiaTemp = spark.read.json("datalake/CovidLiveUpdate/asia.json")
        val caribbeanTemp = spark.read.json("datalake/CovidLiveUpdate/caribbean.json")
        val centralAmericaTemp = spark.read.json("datalake/CovidLiveUpdate/central_america.json")
        val europeTemp = spark.read.json("datalake/CovidLiveUpdate/europe.json")
        val northAmericaTemp = spark.read.json("datalake/CovidLiveUpdate/north_america.json")
        val oceaniaTemp = spark.read.json("datalake/CovidLiveUpdate/oceania.json")
        val southAmericaTemp = spark.read.json("datalake/CovidLiveUpdate/south_america.json")
